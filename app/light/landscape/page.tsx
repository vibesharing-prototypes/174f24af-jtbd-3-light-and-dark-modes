...
function SidebarIcon({ d, active }: { d: string; active?: boolean }) {
  return (
    <div className={`w-7 h-7 rounded-md flex items-center justify-center ${active ? "bg-[#2563eb] text-white" : "bg-[#f3f4f6]"}`}>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={active ? "#fff" : "#6b7280"} strokeWidth="2"><path d={d} /></svg>
    </div>
  );
}
...
function DomainCard({ domain }: { domain: typeof DOMAINS[number] }) {
  return (
    <div className="rounded-xl border border-[#e5e7eb] bg-white p-5 flex flex-col shadow-sm">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-md bg-[#f3f4f6] flex items-center justify-center">
            {domain.title === "Policy Manager" && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><path d="M14 2v6h6" /></svg>}
            {domain.title === "Speak Up" && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" /></svg>}
            {domain.title === "Training" && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z" /><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z" /></svg>}
          </div>
          <h3 className="text-sm font-bold text-[#111827]">{domain.title}</h3>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-[#16a34a]" />
          <span className="text-[10px] text-[#9ca3af]">{domain.lastUpdated}</span>
        </div>
      </div>

      {/* Month + day dots */}
      <div className="mb-1">
        <p className="text-[11px] text-[#6b7280] text-center font-medium mb-2">{domain.month}</p>
          ...
          {Array.from({ length: 31 }, (_, i) => {
            ...
            return (
              <div
                ...
                style={{
                  background: isToday ? "#111827" : isActive ? "#3b82f6" : "#e5e7eb",
                }}
              />
            );
          })}
      ...
      {/* Sparkline chart */}
      <div className="my-3">
        <p className="text-[9px] font-bold uppercase tracking-widest text-[#9ca3af] mb-2">{domain.metric}</p>
        <div className="h-12 relative">
          <div className="absolute bottom-0 left-0 right-0 flex justify-between">
            {MONTHS.map((m) => (
              <span key={m} className="text-[7px] text-[#d1d5db]">{m}</span>
            ))}
          </div>
          <svg viewBox="0 0 240 50" className="w-full h-10" preserveAspectRatio="none">
            <defs>
              <linearGradient id={`grad-light-${domain.title}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#3b82f6" stopOpacity="0.1" />
                <stop offset="100%" stopColor="#3b82f6" stopOpacity="0" />
              </linearGradient>
            </defs>
            <path d={`${domain.sparkline} L240,50 L0,50 Z`} fill={`url(#grad-light-${domain.title})`} />
            <path d={domain.sparkline} fill="none" stroke="#3b82f6" strokeWidth="1.5" />
          </svg>
        </div>
      </div>

      {/* Summary */}
      <p className="text-[11px] text-[#6b7280] leading-relaxed flex-1">{domain.summary}</p>
    </div>
  );
}
...
export default function LandscapeLightPage() {
  return (
        ...
        <div className="flex-1 overflow-y-auto bg-[#f9fafb]">
          <div className="max-w-[1100px] mx-auto px-6 py-10">
            ...
            <div className="text-center mb-8">
              <h1 className="text-3xl font-bold text-[#111827] mb-1">Good morning Ronald!</h1>
              <p className="text-base font-medium text-[#6b7280]">ACME&apos;s compliance landscape</p>
              <p className="text-[11px] text-[#16a34a] mt-2">Updated: Mar 11, 2026 at 7:43 AM</p>
            </div>
            ...
            <div className="max-w-[680px] mx-auto mb-8">
              <div className="rounded-xl border border-[#e5e7eb] bg-white p-5 shadow-sm">
                ...
                <div className="flex items-center gap-2 mt-4">
                  <Link href="/light/dashboard" className="inline-flex items-center rounded-full bg-[#2563eb] hover:bg-[#3b82f6] text-white text-[11px] font-semibold px-3.5 py-1.5 transition-colors">
                    View all suggested actions
                  </Link>
                  <Link href="/light/dashboard" className="inline-flex items-center rounded-full border border-[#d1d5db] bg-white hover:bg-[#f3f4f6] text-[#374151] text-[11px] font-semibold px-3.5 py-1.5 transition-colors">
                    View urgent actions
                  </Link>
                </div>
              </div>
            ...
            <div className="max-w-[680px] mx-auto mb-10">
              <p className="text-[13px] text-[#6b7280] mb-2">What would you like to discuss?</p>
              <div className="flex items-center gap-2 rounded-xl border border-[#e5e7eb] bg-white px-4 py-3 shadow-sm">
                <input
                  type="text"
                  ...
                  readOnly
                />
                <button className="shrink-0 w-8 h-8 rounded-full bg-[#2563eb] hover:bg-[#3b82f6] flex items-center justify-center text-white transition-colors">
                  <SendIcon />
                </button>
              </div>
            </div>
